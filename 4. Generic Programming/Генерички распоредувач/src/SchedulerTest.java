...
// vashiot kod ovde
class Timestamp<T> implements Comparable<Timestamp<?>>{ // ovde imav greska kaj implements
                                                        // bidejki ne bese dodadeno
    ...
    public String toString() {
        String asd = time.toString() + " " + element;
        return asd;
    }
}

class Scheduler<T> extends  Timestamp<T>{
    private ArrayList<Timestamp<T>> list;
    ...
    public Timestamp<T> next(){
        LocalDateTime now = LocalDateTime.now();
        ArrayList<Timestamp<T>> timesList = new ArrayList<>();
        for(int i = 0; i < list.size(); i++) {
            if (list.get(i).getTime().isAfter(now))
    ...
    public Timestamp<T> last(){
        ...
        ArrayList<Timestamp<T>> timesList = new ArrayList<>();
        for(int i = 0; i < list.size(); i++) {
            if (list.get(i).getTime().isBefore(now))
                timesList.add(list.get(i));
        }
        Collections.sort(timesList);
        return timesList.get(timesList.size() - 1);
    }

    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        ArrayList<Timestamp<T>> newarr = new ArrayList<>();
